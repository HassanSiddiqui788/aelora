...
const CATEGORY_META = {
  scarves: {
    title: "Luxe Scarves",
    description: "Brushed cashmere and mulberry silk scarves, designed to hang with signature elegance.",
  },
  hijabs: {
    title: "Premium Hijabs",
    description: "Lightweight, breathable double-sided chiffon and cotton-silk daily accessories.",
  },
  abayas: {
    title: "Signature Abayas",
    description: "Relaxed silhouettes and sophisticated Moroccan crepe tailors, draping with premium ease.",
  },
  accessories: {
    title: "Minimal Details",
    description: "Curated accents to finish and complete your minimal look.",
  },
} as const;
...
export default function ShopCategoryPage({ params }: CategoryPageProps) {
  ...
  const categoryKey = rawCategory.toLowerCase() as ProductCategory;
  const isCategoryValid = Object.keys(CATEGORY_META).includes(categoryKey);
  ...
  useEffect(() => {
    ...
    const fetchCategoryProducts = async () => {
      setLoading(true);
      try {
        const res = await productService.getProducts({ category: categoryKey });
  ...
  if (!isCategoryValid) {
    return (
      <div className="min-h-screen bg-neutral-55 dark:bg-neutral-1000 flex flex-col items-center justify-center space-y-6 px-4">
        <span className="text-[10px] uppercase tracking-[0.25em] font-label text-[#8B6914] font-medium font-semibold">Discovery Portal</span>
        <h2 className="text-2xl md:text-3xl font-display font-light uppercase tracking-widest text-neutral-900 dark:text-neutral-50 text-center">Category Not Found</h2>
        <p className="text-xs font-body font-light text-neutral-400 max-w-sm text-center">
          The category you are looking for does not exist or has been archived. Check out all options in our main showroom.
        </p>
        <Link href={ROUTES.shop} className="text-xs uppercase tracking-widest font-label border-b border-neutral-905 dark:border-neutral-55 pb-1 max-content">
    ...
    <div className="w-full bg-neutral-50 dark:bg-neutral-1000 min-h-screen pt-28 pb-24">
      <div className="max-w-[90rem] mx-auto px-4 md:px-12 space-y-12">
        ...
        <div>
          <Link
            href={ROUTES.shop}
            className="inline-flex items-center gap-2 text-[10px] uppercase font-label tracking-widest text-neutral-400 hover:text-neutral-900 dark:hover:text-white transition-colors"
          >
            <ArrowLeft size={12} /> Catalog Showroom
          </Link>
        </div>
        ...
        <div className="space-y-8">
          ...
          {products.length === 0 ? (
            <div className="py-24 text-center space-y-2">
              <p className="text-xs uppercase tracking-widest font-label text-neutral-400">No items available</p>
              <p className="text-[11px] font-body text-neutral-500">We are currently curating new pieces for this category. Stay tuned.</p>
            </div>
          ) : (
